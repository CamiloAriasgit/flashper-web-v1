    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
            ...
            <div className="flex items-center justify-between mb-8 sm:mb-12">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Panel</h1>
            ...
            {/* Stats Grid: 2x2 en móvil, 3 en escritorio */}
            ...
            {/* Sección de Movimientos Compacta */}
            <div className="bg-white rounded-[2rem] border border-slate-200/50 shadow-sm overflow-hidden">
                ...
                {/* VISTA MÓVIL Y ESCRITORIO UNIFICADA (Estilo Fila Única) */}
                <div className="divide-y divide-slate-50">
                    {quotes?.map((q) => (
                        <div key={q.id} className="group flex items-center justify-between p-4 sm:px-8 sm:py-5 hover:bg-slate-50 transition-all cursor-pointer">
                            <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
                                ...
                                <div className="truncate">
                                    <Link href={`/view/${q.id}`} target="_blank" className="text-sm font-bold text-slate-900 flex items-center gap-1.5 group-hover:text-blue-600">
                                        {q.client_name}
    ...
    return (
        <div className="bg-white p-4 sm:p-6 rounded-[1.5rem] border border-slate-200/60 shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex flex-col justify-between h-full group hover:border-blue-200 transition-all">
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[10px] font-bold text-slate-300 uppercase tracking-tighter sm:tracking-widest">{label}</span>
            </div>
            <div>
