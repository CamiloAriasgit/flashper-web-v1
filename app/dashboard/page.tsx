        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-lg font-bold text-slate-900 tracking-tight italic">Flashper</span>
            </div>
            ...
            {quotes?.map((q) => (
              <div key={q.id} className="p-6 active:bg-slate-50 transition-colors">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-md font-semibold uppercase tracking-tight border ${
                    q.status === 'pending' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'
                  }`}>
              ...
              <tbody className="divide-y divide-slate-100">
                {quotes?.map((q) => (
                  <tr key={q.id} className="group hover:bg-slate-50/40 transition-all">
                    ...
                    <td className="px-8 py-5">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-semibold uppercase tracking-tight border ${
                        q.status === 'pending' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'
                      }`}>
